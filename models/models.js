...
const {
  utilCheckReviewExist,
  utilCheckUsernameExist,
...
exports.selectReviews = (query) => {
  ...
  let categ = category ? [category] : [];
  return db.query(queryStr, categ).then((reviews) => {
...
exports.insertCommentByReviewId = (review_id, { username, body }) => {
  return utilCheckReviewExist(review_id).then(() => {
